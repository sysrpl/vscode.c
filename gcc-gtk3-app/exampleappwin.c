...
static void search_text_changed(GtkEntry *entry, ExampleAppWindow *win)
{
    ...
    if (gtk_text_iter_forward_search(
            &start, text, GTK_TEXT_SEARCH_CASE_INSENSITIVE, &match_start, &match_end, NULL))
    {
...
static void example_app_window_init(ExampleAppWindow *win)
{
    ...
    g_settings_bind(
        win->settings, "transition", win->stack, "transition-type", G_SETTINGS_BIND_DEFAULT);

    g_object_bind_property(
        win->search, "active", win->searchbar, "search-mode-enabled", G_BINDING_BIDIRECTIONAL);
}
