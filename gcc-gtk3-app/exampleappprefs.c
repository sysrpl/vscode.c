...
static GVariant *font_desc_to_string(
...
static GVariant *pos_to_transition(
    const GValue *value, const GVariantType *expected_type, gpointer user_data)
{
...
static void example_app_prefs_init(ExampleAppPrefs *prefs)
{
    ...
    g_settings_bind_with_mapping(prefs->settings, "font", prefs->font, "font-desc",
        G_SETTINGS_BIND_DEFAULT, string_to_font_desc, font_desc_to_string, NULL, NULL);
    g_settings_bind_with_mapping(prefs->settings, "transition", prefs->transition, "selected",
        G_SETTINGS_BIND_DEFAULT, transition_to_pos, pos_to_transition, NULL, NULL);
}
...
static void example_app_prefs_class_init(ExampleAppPrefsClass *class)
{
    ...
    gtk_widget_class_set_template_from_resource(
        GTK_WIDGET_CLASS(class), "/org/gtk/exampleapp/prefs.ui");
